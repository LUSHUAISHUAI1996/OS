...
static long pwm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
  ...
  case PWM_SET :
    ...
    if(!cm_per_pwm) {
      ...
      iowrite16(/*A REMPLIR*/,cm_per_pwm);
...
static int  pinmuxPWM(void)
{
  ...
  if(!cm_per_gpio) {
    ...
    iowrite32(/*A REMPLIR*/,cm_per_gpio);// p. 1426 : GPIO MUX to PWM (Mode 6)
...
static int initPWM(void)
{
  ...
  if(!cm_per_pwm) {
    ...
    iowrite16(/*A REMPLIR*/,cm_per_pwm); // Very Important This register start in freezed mode.
