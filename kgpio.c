...
MODULE_DESCRIPTION("Simple ioctl gpio driver (char)");//功能描述
MODULE_AUTHOR("Ludovic Saint-Bauzel, ISIR UPMC");//作者
MODULE_LICENSE("GPL");//“GPL”[GNU公共许可证v2或更高版本]
...
static long char_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
  ...
  case PWM_SET :
    break;
  case GPIO_SET:
    gpio_set_value(GPIO_PIN9_14, 1);//设置输出GPIO的值总是可以使用 gpio_direction_output（）完成，但是，如果已知GPIO已经处于输出模式，gpio_set_value（）可能会更有效
    break;
  case GPIO_CLEAR:
...
static int __init pinmuxGPIO(void)
{
 int ret=0;
  ...
  cm_per_gpio = ioremap( (AM33XX_CONTROL_BASE+OFFSET_PIN_9_14)/*A REMPLIR*/,4);//ioremap主要是把寄存器做映射，第一个参数是寄存器的起始物理地址，第二个是要映射的寄存器的范围
  if(!cm_per_gpio) {
    printk("cm_per_gpio_reg ioremap: error\n");
  }
  else {
    regval = ioread32(cm_per_gpio) ; //get GPIO MUX register value
    iowrite32( 0x17/*A REMPLIR*/ ,cm_per_gpio);// p. 1426 : GPIO MUX to GPIO OUTPUT (Mode 7)
  ...
  ret = gpio_request_one(  GPIO_PIN9_14/*A REMPLIR*/ , GPIOF_OUT_INIT_HIGH,
			 DEVICE_NAME " gpio");
  if (ret < 0) {
...
static int __init kernmodex_init(void)
{
  ...
  //ret = gpio_request_one(GPIO_PIN9_14,GPIOF_OUT_INIT_HIGH,DEVICE_NAME "gpio");
  ...
  if (IS_ERR(charClass)){                // Check for error and clean up if there is
    unregister_chrdev(major, DEVICE_NAME);//注销字符设备可以使用unregister_chrdev函数。
