...
int counter=0;
static void test_irq_latency_interrupt_handler(unsigned long ptr)
{
  counter++;
...
static int __init pinmuxIRQ(void)
{
  ...
  cm_per_gpio = ioremap(GPIO0_REGISTER/*A REMPLIR*/,4);
  if(!cm_per_gpio) {
    printk("cm_per_gpio_reg ioremap: error\n");
  }
  else {
    regval = ioread32(cm_per_gpio) ; //get GPIO MUX register value
    iowrite32( /*A REMPLIR*/ ,cm_per_gpio);// p. 1426 : GPIO MUX to GPIO INPUT (Mode 7)
  ...
  ret = gpio_request_one( GPIO_PIN9_14 /*A REMPLIR*/, GPIOF_IN,
			  DEVICE_NAME " gpio");
  if (ret < 0) {
    printk(KERN_ALERT DEVICE_NAME " : failed to request GPIO pin.\n" );
    return ret;
  }
  ret  = gpio_to_irq(/*A REMPLIR*/);
  if (ret < 0) {
